...
class OpponentPool:
    def __init__(self, max_size: int = 5):
        ...
        self.agents: dict[int, callable] = {}

    def add(self, version: int, agent_fn: callable):
        self.versions.append(version)
        self.agents[version] = agent_fn
        while len(self.versions) > self.max_size:
            old = self.versions.pop(0)
            del self.agents[old]

    def pick(self) -> tuple[int, callable]:
        ver = random.choice(self.versions)
        return ver, self.agents[ver]
...
def run_single_game(agent_a, agent_b, swap: bool):
    from kaggle_environments import make
    env = make("orbit_wars", debug=False)
    if swap:
        env.run([agent_b, agent_a]); ai, bi = 1, 0
    else:
        env.run([agent_a, agent_b]); ai, bi = 0, 1
    final = env.steps[-1]
    ra = final[ai].reward or 0; rb = final[bi].reward or 0
    ...
    winner = 0 if ra > rb else (1 if rb > ra else -1)
    return {"winner": winner, "ships_a": sa, "ships_b": sb, "length": len(env.steps) - 1}


def run_arena(args):
    print("=" * 70)
    print("  SolarSentinel — Self-Play Arena")
    print("=" * 70)
    ...
    ver = 0; save_snapshot(ver)
    pool.add(ver, load_agent_from_file(SNAPSHOTS_DIR / "agent_v0.py"))
    current = load_agent_from_file(SUBMISSION_FILE)
    ...
    for g in range(1, args.games + 1):
        ov, opp = pool.pick()
        try:
            res = run_single_game(current, opp, swap=(g % 2 == 0))
        except Exception as e:
            print(f"  [ERROR] Game {g}: {e}"); continue

        elo.update("current", f"v{ov}", res["winner"])
        if res["winner"] == 0: wins += 1
        elif res["winner"] == 1: losses += 1
        else: draws += 1

        ce = elo.get("current"); recent_elos.append(ce); elo.save()

        if g % 10 == 0 or g == 1:
            tag = "WIN " if res["winner"] == 0 else ("LOSS" if res["winner"] == 1 else "DRAW")
            print(f"  Game {g:4d} | vs v{ov:<3d} | {tag} | "
                  f"Ships: {res['ships_a']:5.0f} vs {res['ships_b']:5.0f} | "
                  f"ELO: {ce:7.0f} | W/L/D: {wins}/{losses}/{draws} | "
                  f"{time.time()-t0:.0f}s")

        if g % args.snapshot_every == 0:
            ver += 1; save_snapshot(ver)
            pool.add(ver, load_agent_from_file(SNAPSHOTS_DIR / f"agent_v{ver}.py"))
            elo.ratings[f"v{ver}"] = ce
            print(f"  [POOL] +v{ver} (ELO {ce:.0f})  "
                  f"Pool: [{', '.join(f'v{v}' for v in pool.versions)}]")

        if len(recent_elos) >= args.stagnation:
            if max(recent_elos) - min(recent_elos) < 5.0:
                print(f"\n  [STOP] ELO stagnated. Stopping at game {g}."); break

    elapsed = time.time() - t0
    total = wins + losses + draws
    print("\n" + "=" * 70)
    print(f"  ARENA COMPLETE  |  {total} games  |  {wins}W/{losses}L/{draws}D  "
          f"({wins/max(1,total)*100:.0f}%)  |  ELO: {elo.get('current'):.0f}  |  {elapsed:.0f}s")
    print("=" * 70)
...
def _eval_worker(job):
    ...
    job = (candidate_path, opponent_path, game_idx)
    Returns: (winner, ship_lead)
    """
    cand_path, opp_path, game_idx = job
    try:
        from kaggle_environments import make

        # Load both agents fresh in the subprocess
        cand_agent = load_agent_from_file(cand_path)
        opp_agent = load_agent_from_file(opp_path)

        env = make("orbit_wars", debug=False)
        swap = (game_idx % 2 == 0)
        if swap:
            env.run([opp_agent, cand_agent])
            ci, oi = 1, 0
        else:
            env.run([cand_agent, opp_agent])
            ci, oi = 0, 1

        final = env.steps[-1]
        rc = final[ci].reward or 0
        ro = final[oi].reward or 0
        obs = final[0].observation
        planets = obs.planets if hasattr(obs, "planets") else obs.get("planets", [])
        sc = sum(p[5] for p in planets if p[1] == ci)
        so = sum(p[5] for p in planets if p[1] == oi)

        won = 1 if rc > ro else (0 if ro > rc else 0.5)
        return won, sc - so
    except Exception as e:
        return 0, 0


def evaluate_candidate(cand_path: Path, opp_path: Path, n_games: int,
                       n_workers: int) -> tuple[float, float]:
    """
    Evaluate a candidate agent against an opponent over n_games.
    Returns (win_rate, avg_ship_lead).
    """
    jobs = [(str(cand_path), str(opp_path), i) for i in range(n_games)]

    if n_workers > 1:
        with multiprocessing.Pool(min(n_workers, n_games)) as pool:
            results = pool.map(_eval_worker, jobs)
    else:
        results = [_eval_worker(j) for j in jobs]

    wins = sum(r[0] for r in results)
    leads = sum(r[1] for r in results)
    return wins / max(1, len(results)), leads / max(1, len(results))
...
def run_evolution(args):
    ...
    for gen in range(1, args.generations + 1):
        ...
        for i, cfg in enumerate(population):
            ...
            wr, avg_lead = evaluate_candidate(cand_path, opp_path,
                                              args.games_per_eval, n_workers)
